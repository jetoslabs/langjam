...
from tests.expected import FunctionSchema, FunctionObj, ParametersObj, PropertyObj


async def get_openai_function_defination(func, description: str) -> FunctionSchema:
    func_name = func.__name__
    parameters_obj = await get_func_parameters(func)
...
async def get_func_parameters(func) -> ParametersObj:
    func_parameter_names = await get_func_parameter_names(func)
    for func_parameter_name in func_parameter_names:
        if issubclass(func.__annotations__[func_parameter_name], BaseModel):
            ...
            properties = await get_properties(model_json_schema) # Fields of one function param (param is pydantic obj)
...
async def get_func_parameter_names(func) -> List[str]:
    params: List[str] = []
...
async def get_properties(model_json_schema: Dict[str, Any]) -> Dict[str, PropertyObj]:
    ...
    for property_name in properties_initial_state:
        ...
        de_referenced_property = await get_de_refrenced_property(references, property_value, property_name)
        properties[property_name] = de_referenced_property
    return properties


async def get_de_refrenced_property(references: Dict[str, Any], property_value: Dict[str, Any], property_name: str) -> PropertyObj:
    is_done = False
        ...
        if "anyOf" in expanded_property:
            ...
            for option_dict in option_list:
                if "type" in option_dict:
                    ...
                    expanded_property = option_dict; break
